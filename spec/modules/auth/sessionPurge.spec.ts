...
function fakeService(): AuthService {
    return { purgeExpiredSessions: purge } as unknown as AuthService;
}
...
test('purges once at start, then every hour', async () => {
    const running = startSessionPurge(fakeService());
...
test('stops when asked', async () => {
    const running = startSessionPurge(fakeService(), 1000);
    running.stop();
